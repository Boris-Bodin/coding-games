...
export class BasicIA implements IAInterface {
    public availableAction(gameState: GameState, player: Player): Array<Array<Action>> {
        return []
            .concat(
                gameState.getPotions().filter(c => player.canDelivery(c))
                    .sort((a, b) => b.value() - a.value())
                    .map(c => [new Action(ActionType.BREW, c.id)]))
            .concat(
                player.getSpells().filter(spell => player.canCast(spell, 1))
                    .sort((a, b) => b.value() - a.value())
                    .map(c => {
                        if (!c.repeatable) {
            ...
            .concat(
                gameState.getTomes().filter(c => player.canLearn(c))
                    .sort((a, b) => b.value() - a.value())
                    .map(c => [new Action(ActionType.LEARN, c.id)]))
            .concat([[new Action(ActionType.REST)]]);
